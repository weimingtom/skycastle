...
class BinarySerializer {
  ...
  private def createIdToSerializerMap() = {
    ...
    add( new TypeSerializer( classOf[java.lang.Short] ) {
      def len(value: T) = 2
      def enc(buffer: ByteBuffer, value: T) = buffer.putShort( value.shortValue )
      def dec(buffer: ByteBuffer) = java.lang.Short.valueOf( buffer.getShort )
    })

    add( new TypeSerializer( classOf[java.lang.Integer] ) {
      def len(value: T) = 4
      def enc(buffer: ByteBuffer, value: T) = buffer.putInt( value.intValue )
      def dec(buffer: ByteBuffer) = java.lang.Integer.valueOf( buffer.getInt )
    })

    add( new TypeSerializer( classOf[java.lang.Long] ) {
      def len(value: T) = 8
      def enc(buffer: ByteBuffer, value: T) = buffer.putLong( value.longValue )
      def dec(buffer: ByteBuffer) = java.lang.Long.valueOf( buffer.getLong )
    })
